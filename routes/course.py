...
@course.post('/create', )
async def create_course(course_input: Course, current_user: 
...
async def get_course(course_id: str, current_user: Annotated[User, Security(get_current_active_user, scopes=["user"])]):
...
async def update_course(course_id: str, course_input: Course, current_user: Annotated[User, Security(get_current_active_user, scopes=["user"])]):
    find = db.course.find_one(filter={'course_id': course_id})
    if not find:
